+ function () {
    ...
    var interaction_ = function (components, unions) {
        ...
        nav .intent .thru (tap, function (intent) {
            if (intent [0] === 'prepare') {
                dance .setAttribute ('href', '');
                dance .setAttribute ('href', dance_src);
                i = (i + 1) % backgrounds .length;
                backdrop .setAttribute ('href', backgrounds [i]);
            }
        })
    ...
    window .uis = R .assoc (
        'dance', function (components, unions) {
            ...
            return R .merge ({
                ...
                {
                    back: back_stream,
                    dance: dom .querySelector ('#dance #dance image'),
                    backdrop: dom .querySelector ('#background image')
                },
                {
                    nav: nav
                }
