+ function () {
			...
			return {
				pre: function () {
					...
					var un_text_float = TweenMax .fromTo (text_dom, 0, {
							y: ((ui_info .major .text .y_translation - (ui_info .major .height / 2 - ui_info .minor .height / 2)) - ui_info .major .text .y_translation)
						}, {
							y: 0
						});
	...
	var scrolls_ = function (dom, effective_items, _, reversals) {
		...
		var items_doms = effective_items .map (function (item, n) {
			...
			item_dom .querySelector ('image') .setAttributeNS ('http://www.w3.org/1999/xlink', 'href', item .image);
			item_dom .style .transform = 'translate3d(0,0,0)';
			item_dom .style .willChange = 'transform';
